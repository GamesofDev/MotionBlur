...
void main() {
	...
	vec4 currentPosition = vec4(texture.x * 4.0 - 2.0, texture.y * 2.0 - 1.0, 2.0 * depth - 1.0, 1.0);
	...
	// Stronger motion blur multiplier
	vec2 velocity = (currentPosition - previousPosition).st * 0.03; // was 0.015
	...
	if (hand < 0.9) {
		...
		// Increased number of blur samples
		for (int i = 0; i < 24; ++i, coord += velocity) { // was 12
